...
corrplot(cor_df, 
         type = "upper", #keeps only upper part of matrix
         method = "ellipse", #ellipse gives directionality
         tl.col = "black", 
         tl.cex = 0.5)
...
#when outcome dependent variable is already coded as 0 and 1, it's really nice because you know what they mean. Here, 1 is survival and 0 is survival. If it's not already specified, try to specify it yourself in another column using case_when, generally yes= 1
...
f_5_logodds <-predict(donner_blr, newdata=f_5, type = "link") #type = " link means use the previous actual equation"
f_5_logodds

# 3. Exponentiate the log odds to find ODDS of survival for a 5 year old female:

f_5_odds <- exp(f_5_logodds)
#odds of survival are 4 to 1, so that means about 80% chance of survival
...
```{r new_data}
#create a data frame of new data that has a sequence of ages and sex to make prediction for
...
predicted_probs <- predict(donner_blr, newdata= donner_newdata, type = "response", se.fit = TRUE) #what model you're using comes first (here donner_blr)

# Coerce outcome into data frame. 
graph_data <- data.frame(donner_newdata, predicted_probs$fit, predicted_probs$se.fit) # fit is probability for response variable
...
don_graph <- ggplot(graph_data, aes(x= Age, y= Probability)) +
  geom_line(aes(color = Sex)) +
  geom_ribbon(aes(ymin= Probability - SE, ymax= Probability + SE, fill= Sex), alpha= 0.4) +
  theme_classic()
#geom_smooth is great if you want a confidence interval, geom_ribbon is better for SE
...
```{r solar_data}
si_full <- list.files(pattern = "solar_irradiation_*") %>% # tells R to recognize these files 
  map_df(~read_csv(.)) %>% #read everything that exists before it, in this case, all of those files and put them together in a single dataframe
  clean_names() 

#now we want to do some tidying
...
#lubridate is breaking EVERYTHING
